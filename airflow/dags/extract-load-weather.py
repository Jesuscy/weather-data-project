...
# Session sin cache + con retry correcto
...
def weather_request(url=url, params=params):
    ...
    try:
        ...
        if not responses:
            ...
            raise ValueError("Open-Meteo returned empty response")
        ...
        logger.error(f"Error in weather_request: {e}")
        raise e
...
def upload_to_azure(**kwargs):
    ...
    try:
        ...
        try:
            ...
            file_system_client = service_client.create_file_system_if_not_exists(
                file_system="weather-container-landing"
            )
        ...
        logger.error(f"General error in upload_to_azure {e}")
        raise e
